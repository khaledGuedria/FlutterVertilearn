...
class UserService {
    ...
    await Dio().post(
      ...
      if (response.statusCode == 200) {
       ...
       var userResult = User.full(id, email, password);
        ...
        await SharedPreferences.getInstance().then((sp)  {sp.setString("email", userResult.email); sp.setString("password", userResult.email);});
      }
    });
  ...
  Future<String> getEmail() async {
    late String email;
    await SharedPreferences.getInstance().then((value) => email = value.getString("email")!);
    return email;
  }
